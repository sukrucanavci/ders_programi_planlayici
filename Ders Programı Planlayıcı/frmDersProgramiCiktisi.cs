    ...
    public partial class frmDersProgramiCiktisi : Form
    {
        ...
        private void btnOlustur_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    for (int i = 0; i < frmAna.dbDerslik.GetLength(0); i++)
                    {
                        int sayac = 0;
                        ...
                        object hedef = doc.Bookmarks.get_Item(ref dokumanSonu).Range;
                        Word.Paragraph p2 = doc.Content.Paragraphs.Add(ref hedef);
                        string derslikAd = frmAna.derslikler[i].ad;
                        ...
                        for (int g = 0; g < frmAna.gunSayisi; g++)
                        {
                            ...
                            for (int s = 0; s < frmAna.gunlukDersSayisi;)
                            {
                                if (frmAna.dbDerslik[i, sayac] == null)
                                {
                                    sayac++;
                                    tablo.Cell(g + 2, s + 2).Range.Text = "\n\n";
                                    s++;
                                ...
                                int uzunluk = frmAna.dbDerslik[i, sayac].uzunluk;

                                for (int u = 0; u < uzunluk; u++)
                                {
                                    string text = frmAna.dbDerslik[i, sayac].atananDers.ders.kod + "\n";
                                    foreach (Sinif sinif in frmAna.dbDerslik[i, sayac].atananDers.siniflar)
                                    {
                                        text += sinif.kod + " ";
                                    }
                                    text += "\n";
                                    foreach (Ogretmen ogretmen in frmAna.dbDerslik[i, sayac].atananDers.ogretmenler)
                                    {
                                        text += ogretmen.kod + " ";
                                    }
                        ...
                        foreach (AtananDers ad in frmAna.atananDersler)
                        {
                            if (ad.derslikler.Contains(frmAna.derslikler[i]))
                            {
                                if (dersler.ContainsKey(ad.ders.ad))
                                {
                                    dersler[ad.ders.ad] += ad.tds;
